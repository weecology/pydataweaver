...
def get_replace_columns(dialect):
    ...
    val = clean_input(
        "replace_columns (separated by ';', with comma-separated values) (press return to skip): ",
        split_char=";",
        ignore_empty=True,
    )
    if val == "" or val == []:
...
def create_json():
    ...
    while True:
        ...
        if addtable.lower() in ["n", "no"]:
            ...
            # refer dataweaver.lib.table.Table
...
def edit_json(json_file):
    ...
    Usage: dataweaver edit_json <script_name>
