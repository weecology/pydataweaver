...
from dataweaver.lib.defaults import HOME_DIR, ENCODING
        ...
        if not os.path.exists(dir):
            try:
                ...
                if (current_platform != "windows") and os.getenv("SUDO_USER"):
                    # owner of .dataweaver should be user even when installing
                    # w/sudo
                    pw = pwd.getpwnam(os.getenv("SUDO_USER"))
                    os.chown(dir, pw.pw_uid, pw.pw_gid)
            except OSError:
                print(
                    "The Retriever lacks permission to access the ~/.dataweaver/ directory."
                )
                raise
...
def name_matches(scripts, arg):
    ...
    print('\nThe dataset "{}" ' "isn't currently available in the dataweaver.".format(arg))
    if matches:
